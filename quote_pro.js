...
// 앱을 열 때 이만큼 지났으면 화면에 아무 표시 없이 최신 업체 정보로 맞춰둔다
const 업체조용히_MS = 10 * 60 * 1000;
...
/* ---------- 업체(거래처) ----------
   ...
   목록은 폰에도 저장해 둔다. 저장해 둔 것으로 먼저 보여주고 뒤에서 최신을 받아
   갱신한다(마스터 단가와 같은 방식) - 통신이 안 되는 현장에서도 고르던 업체는
   고를 수 있어야 하고, 현장관리 앱에서 방금 고친 주의사항은 바로 따라와야 한다. */
...
function openVendor() {
  ...
  업체키칸(false);
  // 저장해 둔 목록을 먼저 보여주고(위에서 이미 그렸다) 뒤에서 최신을 받는다.
  // 현장관리 앱에서 주의사항을 고치고 바로 넘어와도 옛날 문구가 뜨지 않게 한다.
  업체목록받기(키);
}

/* 앱을 열 때 한 번, 화면에 아무 표시 없이 업체 정보를 최신으로 맞춘다.
   견적을 쓰는 중에 머리줄 밑 주의사항이 옛날 문구로 남아 있으면 안 된다.
   안 되면 저장해 둔 것을 그대로 쓴다 - 조용히 실패해야 한다. */
function 업체조용히맞추기() {
  const 키 = 업체키();
  if (!키) return;
  if (Date.now() - (업체캐시().받은시각 || 0) < 업체조용히_MS) return;
  업체받기(키).catch(() => { /* 통신이 안 되면 저장해 둔 목록 그대로 */ });
}

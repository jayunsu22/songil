...
(function (root, factory) {
  ...
  function 후보(구역, 품목명, 방수) {
    ...
    // 거실 가벽은 예전부터 '가벽' 이라는 이름으로 있었다. 다른 구역은 '주방가벽' 처럼 앞머리를 붙인다.
    if (이름 === '가벽') return { 후보: 구역 === '거실' ? ['가벽', '거실가벽'] : [P + '가벽'] };
    if (/^신발장(틀|문짝)$/.test(이름)) return { 후보: ['신발장1'] };
    ...
    /* 그 밖: 앞머리 + 띄어쓰기 뺀 이름 (방1 '침대 프레임' -> '안방침대프레임').
       2026-09-26 에 짝이 없던 견적 품목들을 현장관리자에 이 이름 규칙으로 추가했다.
       나중에 현장관리자에 같은 규칙으로 품목을 추가하면 코드 수정 없이 짝이 맞는다. */
    return { 후보: [P + 이름.replace(/\s+/g, '')] };
  }

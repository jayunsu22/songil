...
(function () {
  ...
  function 가까운번호(목록, 번호) {
    var 것 = [];
    목록.forEach(function (p) {
      var n = 코드숫자(p.코드);
      if (n != null) 것.push({ n: n, 코드: p.코드 });
    });
  ...
  function 검색() {
    ...
    if (글자) {
      ...
      // "영림 100번". 숫자로 비교해야 PS010 이 10번으로 잡힌다.
      var 번호 = 번호읽기(쪼갬.나머지);
      if (번호 !== null) {
        var 번목 = 자르기(후보.filter(function (p) { return 코드숫자(p.코드) === 번호; }));
        if (번목.length) {
          그리기(번목, (쪼갬.제조사 ? '<b>' + 쪼갬.제조사 + ' ' + 번호 + '번</b> · ' : '<b>' + 번호 + '번</b> · ') +
                       번목.length + '개');
          return;
        }
      }

    ...
    if (글자) {
      ...
      if (번호 !== null) {
        var 별명목 = 후보.filter(function (p) { return 별명번호(p, 쪼갬.제조사) === 번호; })
                        .sort(M.훑어보기정렬(null));
        var 코드목 = 후보.filter(function (p) {
          return 코드숫자(p.코드) === 번호 && 별명번호(p, 쪼갬.제조사) !== 번호;
        }).sort(M.훑어보기정렬(null));
        var 전부 = 별명목.length + 코드목.length;
        if (전부) {
          var 번목 = 별명목.concat(코드목).slice(0, 결과개수);
          결과그리기(번목.map(function (p) {
            // 왜 이게 나왔는지 카드에 적어준다. PW1102 만 덩그러니 있으면
            // 왜 6번으로 나왔는지 알 수 없고, 코드에 우연히 숫자가 든 것
            // (영림 100번을 찾았는데 나온 PS100)은 별명과 헷갈린다.
            var 별 = 별명번호(p, 쪼갬.제조사);
            return {
              제품: p,
              등급: null,
              곁말: 별 === 번호 ? p.색상명 : (별명목.length ? '코드에 ' + 번호 : null),
              곁흐림: 별 !== 번호,
            };
          }), '<b>' + (쪼갬.제조사 ? 쪼갬.제조사 + ' ' : '') + 번호 + '번</b> · ' + 몇개(전부));
          return;
        }
      }
      ...
      // 없는 번호를 부를 때 '없습니다'로 끝내면 오타인지 진짜 없는 건지 모른다.
      if (!목.length && 번호 !== null && 쪼갬.제조사) {
        var 가까 = 가까운번호(후보, 번호, 쪼갬.제조사);
        결과그리기([], '', {
          안내: 쪼갬.제조사 + '에 ' + 번호 + '번은 없습니다.' +
                (가까.length ? ' 가까운 번호: ' + 가까.join(', ') : ''),
          버튼: 쪼갬.제조사 + ' 전체 보기 (' + 후보.length + '개)',
          동작: function () {
            글자 = 쪼갬.제조사;
            $('글자입력').value = 쪼갬.제조사;
            검색();

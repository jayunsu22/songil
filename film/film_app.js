...
(function () {
  ...
  var 팬톤표 = [];      // pantone.json. 사장님이 에어테이블에 채운 번호만 들어 있다
  var 팬톤 = null;      // 이번 검색이 팬톤 번호였다면 그 항목
  var 글자 = '';        // 코드·이름 입력
  var 필터 = {};        // { 제조사: [...] }
  ...
  Promise.all([
    ...
    fetch('pantone.json').then(function (r) { return r.ok ? r.json() : null; })
      .catch(function () { return null; }),   // 팬톤표가 비어 있어도 마찬가지다
  ])
    .then(function (셋) { 질감붙이기(셋[0], 셋[1]); 팬톤표 = 셋[2] || []; 시작(셋[0]); })
    .catch(function (e) {
  ...
  /* ---------- 팬톤 번호 ----------
     ...
     인테리어 업체는 색을 팬톤 번호로 던지는 일이 잦다("15-1247 TCX 로 해주세요").

     번호에서 색을 계산해 낼 방법은 없다. RGB·HSL 처럼 수식이 있는 게 아니라
     잉크 배합을 적어둔 목록의 일련번호라서, 실물 칩을 재어 만든 표를 봐야 한다.
     그 표가 팬톤이 라이선스를 파는 물건이므로 통째로 옮겨오지 않는다.
     고객이 실제로 물어본 번호만 사장님이 에어테이블에 채운다.

     그래서 등록 안 된 번호는 조용히 '없음'으로 끝내지 않고 안내를 띄우고 로그에 남긴다.
     그 로그가 '무엇부터 채워야 하나' 를 알려준다 — 짐작이 아니라 수요가 표를 만든다. */

  // 사람은 'PANTONE 15-1247 TCX', '15-1247tcx', '15 1247 TCX' 를 다 같은 뜻으로 쓴다.
  function 팬톤열쇠(글) {
    return String(글 || '').toUpperCase().replace(/PANTONE/g, '').replace(/[^A-Z0-9]/g, '');
  }

  function 팬톤찾기(글) {
    var k = 팬톤열쇠(글);
    if (!k) return null;
    for (var i = 0; i < 팬톤표.length; i++) {
      var e = 팬톤표[i];
      // HEX 는 그대로 style 에 들어가므로 모양을 여기서 한 번 더 확인한다.
      // 빌드 때도 거르지만, 거르는 곳이 한 군데뿐이면 언젠가 그 한 군데가 빠진다.
      if (e.열쇠 === k && /^#[0-9A-Fa-f]{6}$/.test(e.HEX || '')) return e;
    }
    return null;
  }

  // 표에 없는 글자가 팬톤 번호인지 아닌지 가려낸다. 우리 필름 코드(PS010, TG 4024,
  // HSF124443)와 겹치지 않는 모양만 잡는다 — 겹치면 코드 검색이 먹통이 된다.
  function 팬톤같은가(글) {
    var t = String(글 || '').toUpperCase().trim();
    if (/팬톤|PANTONE/.test(t)) return true;
    if (/^\d{2}\s*-\s*\d{4}(\s*(TCX|TPG|TN|TSX))?$/.test(t)) return true;   // 15-1247 TCX
    if (/^\d{3,4}\s*(C|U|CP|UP)$/.test(t)) return true;                        // 185 C, 2925 U
    if (/^(COOL|WARM)\s*GRAY\s*\d{1,2}\s*(C|U)?$/.test(t)) return true;      // Cool Gray 5 C
    return false;
  }

  // 팬톤은 무늬가 없는 한 가지 색이다. 우드·스톤 무늬에 갖다 대면 뜻이 없으므로
  // 솔리드에서만 찾는다. 재어 보니 이렇게 좁혀도 결과는 거의 나빠지지 않는다
  // (무채색 계열 ΔE 중앙값 2.5 -> 2.7, 흐린 색 4.8 -> 5.2).
  function 솔리드들() {
    return 전체.filter(function (p) { return p.카테고리 === '솔리드'; });
  }

  function 검색() {
    팬톤 = null;   // 지난 검색이 팬톤이었어도 이번 것은 아직 모른다

    // 코드·이름 검색은 색과 별개다. 코드를 아는 사람은 색이 필요 없다.
    if (글자) {
      팬톤 = 팬톤찾기(글자);

      // 등록된 팬톤 번호면 코드가 아니라 색으로 찾는다.
      if (팬톤) {
        var 팬랩 = C.hex를lab(팬톤.HEX);
        var 팬결과 = M.검색(솔리드들(), { lab: 팬랩 }, 필터, { 개수: 결과개수 });
        결과그리기(팬결과, 팬톤머리(팬결과));
        return;
      }
      ...
      // 코드로도 안 잡히는데 팬톤 번호처럼 생겼으면, '없음' 으로 끝내지 않고 알려준다.
      if (!목.length && 팬톤같은가(글자)) {
        집계보내기('팬톤검색', 글자);
        결과그리기([], '', {
          안내: '‘' + 글자 + '’ 는 아직 등록되지 않은 팬톤 번호입니다. ' +
                '문의해 주시면 색을 재어 넣겠습니다. 그동안은 사진으로 찾아보세요.',
          버튼: '사진으로 찾기',
          동작: function () { $('사진입력').click(); },
        });
        return;
      }
  ...
  // 팬톤 검색 결과 머리말. 색칩과 번호를 같이 보여준다 —
  // 번호만 적혀 있으면 화면의 필름들이 왜 나왔는지 알 수 없다.
  // 가장 가까운 것이 얼마나 가까운지도 솔직히 적는다. '없다' 는 것도 답이다.
  function 팬톤머리(결과) {
    var 칩 = '<span class="팬톤칩" style="background:' + 팬톤.HEX + '"></span>';
    var 이름 = 'PANTONE ' + 이스케이프(팬톤.번호);
    if (!결과.length) {
      return 칩 + '<b>' + 이름 + '</b> — 가까운 솔리드 필름이 없습니다';
    }
    var 첫 = 결과[0];
    var 얼마나 = 첫.등급 ? 첫.등급 : (첫.ΔE < 8 ? '조금 다름' : '꽤 다름');
    return 칩 + '<b>' + 이름 + '</b> 과(와) 가까운 솔리드 ' + 결과.length + '개 · ' +
           '가장 가까운 것은 <b>' + 얼마나 + '</b>' +
           (팬톤.출처 === '찾은값' ? ' <span class="팬톤주의">(찾은 색값이라 실물과 다를 수 있습니다)</span>' : '');
  }
  ...
  function 결과그리기(목록, 머리, 제안) {
    ...
    if (!목록.length) {
      ...
      빈.textContent = 팬톤
        ? '이 팬톤 색과 가까운 솔리드 필름이 없습니다. 필름에 없는 색입니다.'
        : (글자
            ? '해당하는 코드·이름이 없습니다.'

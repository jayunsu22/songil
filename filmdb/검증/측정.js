...
async function 재기(옵션 = {}) {
  ...
  const 검색옵션 = { 개수: 옵션.개수 ?? 30, 대비가중치: 옵션.대비가중치, kL: 옵션.kL, 상한: 옵션.상한, 대표톤비중: 옵션.대표톤비중, 무늬어긋남벌점: 옵션.무늬어긋남벌점 };
  const 기록 = [];
  for (const 쪽 of 점목록) {
    ...
    for (const pt of 쪽.점) {
      ...
      const cx = Math.round(pt.x * 사진.폭), cy = Math.round(pt.y * 사진.높이);
      let 색 = 원영역(사진, cx, cy, 반지름, 계수);
      if (!색) { 기록.push({ 쪽: 쪽.쪽, 코드: pt.코드, 순위: null, 사유: '색 못 뽑음' }); continue; }
      // 앱과 똑같이: 결이 있는 구역이면 원을 두 배로 키워 다시 읽는다.
      if (옵션.넓은원 === true && 색.대비폭 >= M.기본값.무늬문턱) {   // 앱은 안 쓴다. 실험용.
        const 넓은 = 원영역(사진, cx, cy, 반지름 * 2, 계수);
        if (넓은) 색 = 넓은;
      }
      const 질의 = 옵션.세톤 === false
        ? { lab: 색.대표색, 대비폭: 색.대비폭 }
        : { lab: 색.대표색, 밝은: 색.밝은색, 어두운: 색.어두운색, 대비폭: 색.대비폭 };
      ...
      기록.push({
        ...
        상위카테고리: 결과.slice(0, 10).map(r => r.제품.카테고리),   // 우드 사진에 솔리드가 끼는지 보려고
      });
    }
  }
  return 기록;
